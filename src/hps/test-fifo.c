...
main()
{
	...
	void *fifo_f2h_base, *fifo_f2h_csr_base;
	...
	for (int i = 1; i < 1024; i++) {
		printf("%d\n", i);
		fifo_write(fifo_h2f_ctx, outbuf, 4);
		FD_ZERO(&fds);
		FD_SET(fifo_f2h_ctx->uio_fd, &fds);
		select(fdmax, &fds, NULL, NULL, &zero_time);
		if (FD_ISSET(fifo_f2h_ctx->uio_fd, &fds)) {
			len = read(fifo_f2h_ctx->uio_fd, &tmp, 4);
			if (len != 4)
